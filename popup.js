...
function loadAllImages() {
  ...
  function renderTime() {
    ...
    // Hours
    ctx.beginPath();
    ctx.arc(120, 130, 100, degToRad(270), degToRad((hours*30)-90));
    ctx.stroke();
    
    // Minutes
     ctx.beginPath();
    ctx.arc(120, 130, 75, degToRad(270), degToRad((minutes*6)-90));
    ctx.stroke();
    // Seconds
     ctx.beginPath();
    ctx.arc(120, 130, 50, degToRad(270), degToRad((newSeconds*6)-90));
    ctx.stroke();
    // Date 
    // ctx.font = "15px Helvetica";
    // ctx.fillStyle = '#28d1fa';
    // ctx.fillText(today, 60, 190);
    
    // Time
    ctx.font = "12px Helvetica";
    ctx.fillStyle = '#28d1fa';
    ctx.fillText(time, 85, 140);
...
window.displayAlarmAnimation = function () {
  ...
  function ring() {
    ...
    currentClockImage =
      rings % 2 == 0 ? blankClockAnim1Image : blankClockAnim2Image;
    drawClock();
    rings--;
  ...
  if (chrome && chrome.tts) {
    chrome.tts.getVoices(function (va) {
      ...
      for (var i = 0; i < voiceArray.length; i++) {
        ...
        console.log(voiceArray[i].voiceName, i);
      }
    });
  }
